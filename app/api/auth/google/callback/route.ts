...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    console.log("Google token exchange status:", tokenRes.status, tokens.error ?? "ok");
    ...
    console.log("Google user email:", googleUser.email ?? "none");
    ...
    if (existingProfile) {
      ...
      console.log("Google callback: found existing user", userId);
    } else {
      ...
      console.log("Google callback: created new user", userId);
    ...
    if (sessionError || !sessionData?.session) {
      console.error("Google callback createSession error:", sessionError?.message);
