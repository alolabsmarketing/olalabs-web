...
function LoginForm() {
  ...
  const [error, setError] = useState(urlError ? (T.errors[urlError as keyof typeof T.errors] ?? "An error occurred.") : "");
  ...
  return (
    <div className="rounded-2xl bg-[#111] border border-white/8 p-8">
      ...
      <form onSubmit={handleLogin} className="space-y-4">
        ...
        >
          {loading ? T.signingIn : <>{T.signIn} <ArrowRight size={14} /></>}
        </button>
      </form>
    ...
    <div className="bg-[#080808] flex min-h-screen items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="relative text-center mb-8">
          ...
          <p className="text-white/60 text-sm mt-2">{T.welcomeBack}</p>
